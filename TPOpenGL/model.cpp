...
void Model::loadModel(string const &path)
{
	...
	const aiScene* scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_JoinIdenticalVertices | aiProcess_OptimizeGraph | aiProcess_OptimizeMeshes | aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace);
...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
	...
	for (unsigned int i = 0; i < mesh->mNumVertices; i++)
	{
		...
		// normals
		vector.x = mesh->mNormals == nullptr ? 0.0 : mesh->mNormals[i].x;
		vector.y = mesh->mNormals == nullptr ? 0.0 : mesh->mNormals[i].y;
		vector.z = mesh->mNormals == nullptr ? 0.0 : mesh->mNormals[i].z;
		...
		// tangent
		vector.x = mesh->mTangents == nullptr ? 0.0 : mesh->mTangents[i].x;
		vector.y = mesh->mTangents == nullptr ? 0.0 : mesh->mTangents[i].y;
		vector.z = mesh->mTangents == nullptr ? 0.0 : mesh->mTangents[i].z;
		vertex.Tangent = vector;
		// bitangent
		vector.x = mesh->mBitangents == nullptr ? 0.0 : mesh->mBitangents[i].x;
		vector.y = mesh->mBitangents == nullptr ? 0.0 : mesh->mBitangents[i].y;
		vector.z = mesh->mBitangents == nullptr ? 0.0 : mesh->mBitangents[i].z;
